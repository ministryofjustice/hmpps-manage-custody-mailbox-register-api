...
class AuditLoggingCreationTest {
  ...
  @Test
  fun `Logs creation actions as CREATE`() {
    AuditLog(auditLogEntryRepository).logCreationOf(auditableEntity)
    assertThat(savedEntry?.action).isEqualTo(AuditAction.CREATE)
  }

  @Test
  fun `Creation actions have each auditable field saved in the updates field`() {
    AuditLog(auditLogEntryRepository).logCreationOf(auditableEntity)

    assertThat(savedEntry?.updates).isEqualTo(
      mapOf(
        "name" to listOf(null, "Mr Goodname"),
        "email" to listOf(null, "goodemail@example.com"),
        "age" to listOf(null, 35),
      ),
    )
  }
  ...
  fun `username can be overridden`() {
    AuditLog(auditLogEntryRepository).logCreationOf(auditableEntity, "A_GIVEN_USERNAME")
